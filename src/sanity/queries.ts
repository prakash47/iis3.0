...
/**
 * GROQ queries. Every read filters `seo.noindex != true` so thin/unpublished-quality
 * docs never enter a listing, the sitemap, or generateStaticParams. Doc-body image
 * blocks project real asset dimensions + lqip so next/image renders with zero CLS.
 */
...
export const POST_SLUGS = defineQuery(
  `*[_type == "post" && defined(slug.current) && defined(publishedAt) && seo.noindex != true].slug.current`,
);
...
export const RELATED_POSTS = defineQuery(`
  *[_type == "post" && slug.current != $slug && category->slug.current == $categorySlug
    && defined(publishedAt) && seo.noindex != true] | order(publishedAt desc)[0...3]{
      "slug": slug.current, title, excerpt }`);

// ---- Guides ----
export const GUIDE_SLUGS = defineQuery(
  `*[_type == "guide" && defined(slug.current) && seo.noindex != true].slug.current`,
);

export const GUIDE_INDEX = defineQuery(`
  *[_type == "guide" && seo.noindex != true] | order(coalesce(publishedAt, _updatedAt) desc){
...
export const GLOSSARY_ALL = defineQuery(`
  *[_type == "glossaryTerm" && seo.noindex != true] | order(lower(title) asc){
...
export const RESOURCES_LATEST = defineQuery(`
  *[_type in ["post","guide"] && seo.noindex != true && defined(coalesce(publishedAt, _updatedAt))]
...
export const ALL_RESOURCE_ENTRIES = defineQuery(`
  *[_type in ["post","guide","glossaryTerm"] && seo.noindex != true && defined(slug.current)]{
